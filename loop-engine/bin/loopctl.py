...
def ensure_product_runtime_gitignore(repo_path: Path) -> bool:
    path = repo_path / ".gitignore"
    lines = path.read_text().splitlines() if path.exists() else []
    present = {line.strip() for line in lines}
    missing = [line for line in PRODUCT_RUNTIME_GITIGNORE_LINES if line not in present]
    if not missing:
        return False
    if lines and lines[-1].strip():
        lines.append("")
    lines.append("# Loop runtime handoff artifacts.")
...
# Runtime handoff files created by `loop approve` in the product repo. They are
# inputs for the control plane, not product source, so bootstrap makes them
# git-ignored before the first clean-tree gate can reject a normal first run.
PRODUCT_RUNTIME_GITIGNORE_LINES = (".loop/daily-focus/",)

# Generated by common Python verification commands. These files should not make
# a safe task fail the Allowed-Files gate, while real source changes still do.
GENERATED_STATUS_DIRS = ("__pycache__/", ".pytest_cache/")
GENERATED_STATUS_SUFFIXES = (".pyc", ".pyo")
...
def changed_files(worktree_path: Path) -> list[str]:
    ...
    return [f for f in files if f and not generated_status_noise(f)]


def generated_status_noise(path: str) -> bool:
    nf = _norm_path(path)
    if nf.endswith(GENERATED_STATUS_SUFFIXES):
        return True
    return any(nf == d.rstrip("/") or nf.startswith(d) or f"/{d}" in f"/{nf}" for d in GENERATED_STATUS_DIRS)
